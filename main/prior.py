...
class RepeatConstraint(Constraint):
    ...
    def is_violated(self, actions, parent, sibling):
        counts = np.sum(np.isin(actions, self.tokens))
        return counts < self.min or counts > self.max if self.min is not None and self.max is not None else False
